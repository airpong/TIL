...
export default class App extends React.Component {
  ...
  _openAuthSessionAsync = async () => {
    ...
    const URL = `http://ec2-3-34-162-241.ap-northeast-2.compute.amazonaws.com:8080/oauth2/authorize/google?redirect_uri=${Linking.makeUrl(
      ""
    )}`;
    try {
      this._addLinkingListener();
  ...
  _openBrowserAsync = async () => {
    console.log("Im in browserAsync");
    const URL = `http://ec2-3-34-162-241.ap-northeast-2.compute.amazonaws.com:8080/oauth2/authorize/google?redirect_uri=${Linking.makeUrl(
      ""
    )}`;
    try {
